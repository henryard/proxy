...
convert them into Loon-compatible ``.list`` rule-set files.

sing-box JSON  ->  Loon .list
    domain          -> DOMAIN,<value>
    ...
    domain_regex    -> skipped (not supported by Loon rule-sets)

This mirrors scripts/singbox_to_shadowrocket.py exactly (same ruleset list,
same source URLs, same fetch/retry logic) so the two outputs stay in sync -
Loon's plain-text rule-set syntax is identical to Shadowrocket's, only the
...
Usage:
    # Convert the built-in list of rulesets and write into ./Loon/ruleset
    python3 singbox_to_loon.py --out Loon/ruleset

    # Convert a single local file or URL to stdout
    python3 singbox_to_loon.py --one openai.json
"""
...
# Kept identical to scripts/singbox_to_shadowrocket.py so both outputs cover
...
USER_AGENT = "singbox-to-loon/1.0 (+https://github.com)"
...
def render(name, source_url, lines, counts):
    ...
    header = [
        f"# Loon rule-set: {name}",
        f"# Converted from sing-box: {source_url}",
        f"# Updated: {now} (Beijing / UTC+8)",
        f"# Rules: {summary}",
    ]
    if counts.get("regex_skipped"):
        header.append(f"# Note: skipped {counts['regex_skipped']} domain_regex rule(s) (unsupported by Loon)")
    body = "\n".join(header + [""] + lines)
    return body.rstrip("\n") + "\n"
...
def main():
    parser = argparse.ArgumentParser(description="Convert sing-box geosite rule-sets to Loon .list format")
    parser.add_argument("--out", default="Loon/ruleset", help="output directory for batch mode")
